...
test('two browsers join the same room and camera changes do not interrupt W movement', async ({ browser }) => {
  ...
  await expect(pageA.locator('#roomInfo')).toHaveAttribute('data-visible', 'true', { timeout: 15000 });
  ...
  await expect(pageA.locator('#memberCount')).toHaveText('2 / 8', { timeout: 15000 });
  await expect(pageB.locator('#memberCount')).toHaveText('2 / 8', { timeout: 15000 });

  await pageA.locator('.portalBtn[data-map="color"]').click();
  await expect(pageA.locator('#lobbyOverlay')).toHaveAttribute('data-visible', 'false', { timeout: 10000 });
  await expect(pageB.locator('#lobbyOverlay')).toHaveAttribute('data-visible', 'false', { timeout: 10000 });

  await pageB.waitForFunction(() => Boolean(window.__TRIPLE_OBBY_RUNTIME__));
